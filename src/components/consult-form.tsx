...
export function ConsultForm() {
  ...
  return (
    <div className="border border-white/10 bg-white/[0.04] p-8">
      <input
        className="w-full border-b border-white/20 bg-transparent py-5 text-sm outline-none placeholder:text-slate-500"
      ...
      {isFilled ? (
        ...
        <button disabled type="button" className="btn-primary mt-8 w-full cursor-not-allowed opacity-40 flex items-center justify-center gap-3">
          <WhatsAppIcon />
          Получить консультацию
        </button>
      )}
      <p className="mt-7 text-center text-[11px] leading-5 text-slate-500">Нажимая на кнопку, вы соглашаетесь с политикой конфиденциальности</p>
    </div>
  )
}
